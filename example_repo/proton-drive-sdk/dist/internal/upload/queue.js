 ...
 * to network issues.
 */
const MAX_CONCURRENT_UPLOAD_SIZE = 10 * streamUploader_1.FILE_CHUNK_SIZE;
/**
...
class UploadQueue {
    totalFileUploads = 0;
    totalExpectedSize = 0;
    async waitForCapacity(expectedSize, signal) {
        await (0, wait_1.waitForCondition)(() => this.totalFileUploads < MAX_CONCURRENT_FILE_UPLOADS &&
            this.totalExpectedSize < MAX_CONCURRENT_UPLOAD_SIZE, signal);
        this.totalFileUploads++;
        this.totalExpectedSize += expectedSize;
    }
    releaseCapacity(expectedSize) {
        this.totalFileUploads--;
        this.totalExpectedSize -= expectedSize;
    }
}
