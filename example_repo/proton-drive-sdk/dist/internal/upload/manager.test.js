...
describe('UploadManager', () => {
    ...
    beforeEach(() => {
        ...
        manager = new manager_1.UploadManager(telemetry, apiService, cryptoService, nodesService, clientUid);
    });
    describe('createDraftNode', () => {
        it('should fail to create node in non-folder parent', async () => {
            ...
            const result = manager.createDraftNode('parentUid', 'name', {});
        ...
        it('should delete existing draft and trying again', async () => {
            ...
            apiService.createDraft = jest.fn().mockImplementation(() => {
                if (firstCall) {
                    firstCall = false;
                    throw new errors_1.ValidationError('Draft already exists', 2500 /* ErrorCode.ALREADY_EXISTS */, {
            ...
            const result = await manager.createDraftNode('volumeId~parentUid', 'name', {});
            expect(result).toEqual({
                nodeUid: 'newNode:nodeUid',
                nodeRevisionUid: 'newNode:nodeRevisionUid',
                nodeKeys: {
        ...
        it('should not delete existing draft if client UID does not match', async () => {
            ...
            const promise = manager.createDraftNode('volumeId~parentUid', 'name', {});
            try {
                await promise;
            }
            catch (error) {
        ...
        it('should handle error when deleting existing draft', async () => {
            ...
            const result = manager.createDraftNode('volumeId~parentUid', 'name', {});
            try {
                await result;
            }
            catch (error) {
    ...
    describe('commit draft', () => {
        ...
        it('should commit revision draft', async () => {
            await manager.commitDraft(nodeRevisionDraft, manifest, extendedAttributes);
        ...
        it('should commit node draft', async () => {
            ...
            await manager.commitDraft(nodeRevisionDraftWithNewNodeInfo, manifest, extendedAttributes);
            expect(cryptoService.commitFile).toHaveBeenCalledWith(nodeRevisionDraft.nodeKeys, manifest, expect.anything());
            expect(apiService.commitDraftRevision).toHaveBeenCalledWith(nodeRevisionDraft.nodeRevisionUid, expect.anything());
        ...
        it('should throw error if revision was not committed successfully', async () => {
            ...
            await expect(manager.commitDraft(nodeRevisionDraft, manifest, extendedAttributes)).rejects.toThrow('Revision to commit must be a draft');
            expect(nodesService.notifyNodeChanged).not.toHaveBeenCalled();
        });
        it('should throw original error if revision cannot be verified', async () => {
            ...
            await expect(manager.commitDraft(nodeRevisionDraft, manifest, extendedAttributes)).rejects.toThrow('Failed to commit revision');
            expect(nodesService.notifyNodeChanged).not.toHaveBeenCalled();
        });
    });
});
