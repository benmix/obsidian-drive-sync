...
describe('ChunkStreamReader', () => {
    ...
    it('should yield chunks as enqueued if matching the size', async () => {
        const reader = new chunkStreamReader_1.ChunkStreamReader(stream, 3);
        const chunks = [];
    ...
    it('should yield smaller chunks than enqueued chunks', async () => {
        const reader = new chunkStreamReader_1.ChunkStreamReader(stream, 2);
        const chunks = [];
        for await (const chunk of reader.iterateChunks()) {
            chunks.push(new Uint8Array(chunk));
        }
    ...
    it('should yield bigger chunks than enqueued chunks', async () => {
        const reader = new chunkStreamReader_1.ChunkStreamReader(stream, 4);
        const chunks = [];
        for await (const chunk of reader.iterateChunks()) {
            chunks.push(new Uint8Array(chunk));
        }
    ...
    it('should yield last incomplete chunk', async () => {
        const reader = new chunkStreamReader_1.ChunkStreamReader(stream, 5);
        const chunks = [];
        for await (const chunk of reader.iterateChunks()) {
            chunks.push(new Uint8Array(chunk));
        }
    ...
    it('should yield as one big chunk', async () => {
        const reader = new chunkStreamReader_1.ChunkStreamReader(stream, 100);
        const chunks = [];
        for await (const chunk of reader.iterateChunks()) {
            chunks.push(new Uint8Array(chunk));
        }
...
//# sourceMappingURL=chunkStreamReader.test.js.map