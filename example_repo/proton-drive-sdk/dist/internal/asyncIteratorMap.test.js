...
describe('asyncIteratorMap', () => {
    ...
    test('throws AbortError if signal is aborted', async () => {
        ...
        ac.abort();
        const mappedGen = (0, asyncIteratorMap_1.asyncIteratorMap)(inputGen, mapper, 1, ac.signal);
        await expect(collectResults(mappedGen)).rejects.toThrow(errors_1.AbortError);
    });
});
