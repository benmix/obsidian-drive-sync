...
exports.NodesCryptoService = void 0;
 ...
 */
class NodesCryptoService {
    driveCrypto;
    account;
    reporter;
    logger;
    constructor(telemetry, driveCrypto, account, reporter) {
    ...
    async decryptNode(node, parentKey) {
        ...
        try {
            ...
            const errorMessage = (0, ttag_1.c)('Error').t `Failed to decrypt node key: ${message}`;
            ...
            return {
                node: {
                    ...
                    keyAuthor: (0, interface_1.resultError)({
                        claimedAuthor: getClaimedAuthor(node.encryptedCrypto.signatureEmail, keyVerificationKeys.length === 0),
                        error: errorMessage,
                    }),
                    nameAuthor,
                    membership,
                    activeRevision: 'file' in node.encryptedCrypto ? (0, interface_1.resultError)(new Error(errorMessage)) : undefined,
                    folder: undefined,
                    errors: [error],
                },
            };
        ...
        if ('folder' in node.encryptedCrypto) {
            ...
            const [hashKeyPromise, folderExtendedAttributesPromise] = [
                ...
                this.decryptExtendedAttributes(node, node.encryptedCrypto.folder.armoredExtendedAttributes, key, folderExtendedAttributesVerificationKeys, node.encryptedCrypto.signatureEmail),
            ];
            try {
        ...
        if ('file' in node.encryptedCrypto) {
            ...
            try {
                activeRevision = (0, interface_1.resultOk)(await activeRevisionPromise);
            }
            catch (error) {
                void this.reporter.reportDecryptionError(node, 'nodeExtendedAttributes', error);
                const message = (0, errors_2.getErrorMessage)(error);
                const errorMessage = (0, ttag_1.c)('Error').t `Failed to decrypt active revision: ${message}`;
                activeRevision = (0, interface_1.resultError)(new Error(errorMessage));
            }
            try {
                ...
                contentKeyPacketAuthor =
                    keySessionKeyResult.verified !== undefined &&
                        (await this.reporter.handleClaimedAuthor(node, 'nodeContentKey', (0, ttag_1.c)('Property').t `content key`, keySessionKeyResult.verified, keySessionKeyResult.verificationErrors, node.encryptedCrypto.signatureEmail));
            }
            catch (error) {
                void this.reporter.reportDecryptionError(node, 'nodeContentKey', error);
                const message = (0, errors_2.getErrorMessage)(error);
                const errorMessage = (0, ttag_1.c)('Error').t `Failed to decrypt content key: ${message}`;
                contentKeyPacketAuthor = (0, interface_1.resultError)({
    ...
    async decryptKey(node, parentKey, verificationKeys) {
        const key = await this.driveCrypto.decryptKey(node.encryptedCrypto.armoredKey, node.encryptedCrypto.armoredNodePassphrase, node.encryptedCrypto.armoredNodePassphraseSignature, [parentKey], verificationKeys);
        return {
            passphrase: key.passphrase,
            key: key.key,
            passphraseSessionKey: key.passphraseSessionKey,
            author: await this.reporter.handleClaimedAuthor(node, 'nodeKey', (0, ttag_1.c)('Property').t `key`, key.verified, key.verificationErrors, node.encryptedCrypto.signatureEmail, verificationKeys.length === 0),
        };
    }
    async decryptName(node, parentKey, verificationKeys) {
        const nameSignatureEmail = node.encryptedCrypto.nameSignatureEmail;
        try {
            const { name, verified: verificationStatus, verificationErrors, } = await this.driveCrypto.decryptNodeName(node.encryptedName, parentKey, verificationKeys);
            ...
            if (verificationStatus === crypto_1.VERIFICATION_STATUS.NOT_SIGNED && node.creationTime < new Date(2021, 0, 1)) {
                verified = crypto_1.VERIFICATION_STATUS.SIGNED_AND_VALID;
            }
            return {
                name: (0, interface_1.resultOk)(name),
                author: await this.reporter.handleClaimedAuthor(node, 'nodeName', (0, ttag_1.c)('Property').t `name`, verified, verificationErrors, nameSignatureEmail, verificationKeys.length === 0),
            };
        }
        catch (error) {
            void this.reporter.reportDecryptionError(node, 'nodeName', error);
            const errorMessage = (0, errors_2.getErrorMessage)(error);
            return {
                name: (0, interface_1.resultError)(new Error(errorMessage)),
                author: (0, interface_1.resultError)({
        ...
        if (node.encryptedCrypto.membership) {
            ...
            try {
                ...
                sharedBy = (0, interface_1.resultError)({
                    ...
                    error: (0, ttag_1.c)('Error').t `Failed to get inviter keys`,
                });
            }
            try {
                const { verified, verificationErrors } = await this.driveCrypto.verifyInvitation(node.encryptedCrypto.membership.base64MemberSharePassphraseKeyPacket, node.encryptedCrypto.membership.armoredInviterSharePassphraseKeyPacketSignature, inviterEmailKeys || []);
                sharedBy = await this.reporter.handleClaimedAuthor(node, 'membershipInviter', (0, ttag_1.c)('Property').t `membership`, verified, verificationErrors, node.encryptedCrypto.membership.inviterEmail);
            }
            catch (error) {
                ...
                sharedBy = (0, interface_1.resultError)({
                    claimedAuthor: node.encryptedCrypto.membership.inviterEmail,
                    error: (0, ttag_1.c)('Error').t `Failed to verify invitation`,
                });
            }
        }
        else {
            sharedBy = (0, interface_1.resultError)({
                error: (0, ttag_1.c)('Error').t `Missing inviter email`,
            });
        }
        return {
    ...
    async decryptHashKey(node, nodeKey, addressKeys) {
        if (!('folder' in node.encryptedCrypto)) {
            // This is developer error.
            throw new Error('Node is not a folder');
        }
        const { hashKey, verified: verificationStatus, verificationErrors, } = await this.driveCrypto.decryptNodeHashKey(node.encryptedCrypto.folder.armoredHashKey, nodeKey, addressKeys);
        let verified = verificationStatus;
        // The hash was not signed until Drive web Beta 17.
        // It is decided to ignore this and consider it signed.
        // The problem will be gone with migration to new crypto model.
        if (verificationStatus === crypto_1.VERIFICATION_STATUS.NOT_SIGNED && node.creationTime < new Date(2021, 7, 1)) {
            verified = crypto_1.VERIFICATION_STATUS.SIGNED_AND_VALID;
        }
        return {
            hashKey,
            author: await this.reporter.handleClaimedAuthor(node, 'nodeHashKey', (0, ttag_1.c)('Property').t `hash key`, verified, verificationErrors, node.encryptedCrypto.signatureEmail),
        };
    }
    async decryptRevision(nodeUid, encryptedRevision, nodeKey) {
        const verificationKeys = encryptedRevision.signatureEmail
            ? await this.account.getPublicKeys(encryptedRevision.signatureEmail)
            : [nodeKey];
        const { extendedAttributes, author: contentAuthor } = await this.decryptExtendedAttributes({ uid: nodeUid, creationTime: encryptedRevision.creationTime }, encryptedRevision.armoredExtendedAttributes, nodeKey, verificationKeys, encryptedRevision.signatureEmail);
        return {
    ...
    async decryptContentKeyPacket(node, encryptedCrypto, key, keyVerificationKeys) {
        const result = await this.driveCrypto.decryptAndVerifySessionKey(encryptedCrypto.file.base64ContentKeyPacket, encryptedCrypto.file.armoredContentKeyPacketSignature, key, 
        ...
        if (result.verified !== crypto_1.VERIFICATION_STATUS.SIGNED_AND_INVALID || node.creationTime > new Date(2023, 0, 1)) {
            return result;
        }
        const allAddresses = await this.account.getOwnAddresses();
        const allKeys = allAddresses.flatMap((address) => address.keys.map(({ key }) => key));
        const resultWithAllKeys = await this.driveCrypto.decryptAndVerifySessionKey(encryptedCrypto.file.base64ContentKeyPacket, encryptedCrypto.file.armoredContentKeyPacketSignature, key, 
        // Content key packet is signed with the node key, but
        // in the past some clients signed with the address key.
        allKeys);
        // Return original result with original error if the fallback verification also fails.
        if (resultWithAllKeys.verified === crypto_1.VERIFICATION_STATUS.SIGNED_AND_VALID) {
            this.logger.warn('Content key packet signature verification failed, but fallback to all addresses succeeded');
            return resultWithAllKeys;
        }
        return result;
    }
    async decryptExtendedAttributes(node, encryptedExtendedAttributes, nodeKey, addressKeys, signatureEmail) {
        if (!encryptedExtendedAttributes) {
            return {
                author: (0, interface_1.resultOk)(signatureEmail),
            };
        }
        const { extendedAttributes, verified, verificationErrors } = await this.driveCrypto.decryptExtendedAttributes(encryptedExtendedAttributes, nodeKey, addressKeys);
        return {
            extendedAttributes,
            author: await this.reporter.handleClaimedAuthor(node, 'nodeExtendedAttributes', (0, ttag_1.c)('Property').t `attributes`, verified, verificationErrors, signatureEmail),
        };
    }
    async createFolder(parentKeys, signingKeys, name, extendedAttributes) {
        const email = signingKeys.type === 'userAddress' ? signingKeys.email : null;
        const nameAndPassprhaseSigningKey = signingKeys.type === 'userAddress' ? signingKeys.key : signingKeys.parentNodeKey;
        ...
        const extendedAttributesSigningKey = signingKeys.type === 'userAddress' ? signingKeys.key : signingKeys.nodeKey || nodeKeys.decrypted.key;
        const { armoredExtendedAttributes } = extendedAttributes
            ? await this.driveCrypto.encryptExtendedAttributes(extendedAttributes, nodeKeys.decrypted.key, extendedAttributesSigningKey)
    ...
    async encryptNewName(parentKeys, nodeNameSessionKey, signingKeys, newName) {
        const email = signingKeys.type === 'userAddress' ? signingKeys.email : null;
        ...
        const { armoredNodeName } = await this.driveCrypto.encryptNodeName(newName, nodeNameSessionKey, parentKeys.key, nameSigningKey);
    ...
    async encryptNodeWithNewParent(nodeName, keys, parentKeys, signingKeys) {
        if (!parentKeys.hashKey) {
            throw new errors_1.ValidationError('Moving item to a non-folder is not allowed');
        }
        if (!nodeName.ok) {
            throw new errors_1.ValidationError('Cannot move item without a valid name, please rename the item first');
        }
        const email = signingKeys.type === 'userAddress' ? signingKeys.email : null;
        ...
        const { armoredNodeName } = await this.driveCrypto.encryptNodeName(nodeName.value, keys.nameSessionKey, parentKeys.key, nameAndPassprhaseSigningKey);
        const hash = await this.driveCrypto.generateLookupHash(nodeName.value, parentKeys.hashKey);
        const { armoredPassphrase, armoredPassphraseSignature } = await this.driveCrypto.encryptPassphrase(keys.passphrase, keys.passphraseSessionKey, [parentKeys.key], nameAndPassprhaseSigningKey);
        return {
            encryptedName: armoredNodeName,
            hash,
    ...
    async generateNameHashes(parentHashKey, names) {
...
exports.NodesCryptoService = NodesCryptoService;
function getClaimedAuthor(claimedAuthor, notAvailableVerificationKeys = false) {
