...
describe('NodesManagement', () => {
    ...
    beforeEach(() => {
        ...
        apiService = {
            ...
            trashNodes: jest.fn(async function* (uids) {
                yield* uids.map((uid) => ({ ok: true, uid }));
            }),
        ...
        // @ts-expect-error No need to implement all methods for mocking
        nodesAccess = {
            getNode: jest.fn().mockImplementation((uid) => nodes[uid]),
        ...
        management = new nodesManagement_1.NodesManagement(apiService, cryptoCache, cryptoService, nodesAccess);
    });
    it('renameNode manages rename and updates cache', async () => {
        ...
        expect(cryptoService.encryptNewName).toHaveBeenCalledWith({ key: 'parentUid-key', hashKey: 'parentUid-hashKey' }, 'nodeUid-nameSessionKey', { type: 'userAddress', email: 'root-email', addressId: 'root-addressId', key: 'root-key' }, 'new name');
        expect(apiService.renameNode).toHaveBeenCalledWith(nodes.nodeUid.uid, { hash: nodes.nodeUid.hash }, { encryptedName: 'newArmoredNodeName', nameSignatureEmail: 'newSignatureEmail', hash: 'newHash' });
    ...
    it('renameNode refreshes cache if node is out of sync', async () => {
        const error = new errors_1.NodeOutOfSyncError('Node is out of sync');
    ...
    it('moveNode manages move and updates cache', async () => {
        ...
        expect(cryptoService.encryptNodeWithNewParent).toHaveBeenCalledWith(nodes.nodeUid.name, expect.objectContaining({
            key: 'nodeUid-key',
        ...
        }), expect.objectContaining({ key: 'newParentNodeUid-key', hashKey: 'newParentNodeUid-hashKey' }), { type: 'userAddress', email: 'root-email', addressId: 'root-addressId', key: 'root-key' });
        expect(apiService.moveNode).toHaveBeenCalledWith('nodeUid', {
    ...
    it('moveNode manages move of anonymous node', async () => {
        ...
        expect(cryptoService.encryptNodeWithNewParent).toHaveBeenCalledWith(nodes.anonymousNodeUid.name, expect.objectContaining({
        ...
        expect(apiService.moveNode).toHaveBeenCalledWith('anonymousNodeUid', {
            hash: nodes.nodeUid.hash,
        }, {
            parentUid: 'newParentNodeUid',
            ...encryptedCrypto,
    ...
    it('copyNode manages copy of node with new name', async () => {
        ...
        expect(cryptoService.encryptNodeWithNewParent).toHaveBeenCalledWith({ ok: true, value: newName }, expect.objectContaining({
            key: 'nodeUid-key',
            passphrase: 'nodeUid-passphrase',
            passphraseSessionKey: 'nodeUid-passphraseSessionKey',
            contentKeyPacketSessionKey: 'nodeUid-contentKeyPacketSessionKey',
    ...
    it('copyNode throws error if name is invalid', async () => {
        ...
        await expect(promise).rejects.toThrow(errors_2.ValidationError);
    });
...
//# sourceMappingURL=nodesManagement.test.js.map