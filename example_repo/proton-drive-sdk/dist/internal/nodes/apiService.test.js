...
function generateAlbumNode(overrides = {}) {
    ...
    return {
        ...node,
        type: interface_1.NodeType.Album,
        ...overrides,
    };
}
...
describe('nodeAPIService', () => {
    ...
    beforeEach(() => {
        ...
        api = new apiService_2.NodeAPIService((0, logger_1.getMockLogger)(), apiMock, 'clientUid');
    });
    describe('getNode', () => {
    ...
    describe('iterateNodes', () => {
        async function testIterateNodes(mockedLink, expectedNode, ownVolumeId = 'volumeId') {
            // @ts-expect-error Mocking for testing purposes
            apiMock.post = jest.fn(async () => Promise.resolve({
                Links: [mockedLink],
            }));
        ...
        it('should get root folder node', async () => {
            await testIterateNodes(generateAPIFolderNode({ ParentLinkID: null }), generateFolderNode({ parentUid: undefined }));
        });
        ...
        it('should get shared node', async () => {
            await testIterateNodes(generateAPIFolderNode({}, {
                Sharing: {
                    ShareID: 'shareId',
                },
                Membership: {
                ...
                membership: {
                    ...
                    armoredInviteeSharePassphraseSessionKeySignature: 'inviteeSharePassphraseSessionKeySignature',
                },
            }));
        });
        it('should get shared node with unknown permissions', async () => {
            await testIterateNodes(generateAPIFolderNode({}, {
                ...
                isSharedPublicly: false,
                shareId: 'shareId',
                directRole: interface_1.MemberRole.Viewer,
                membership: {
        ...
        it('should get trashed file node', async () => {
            await testIterateNodes(generateAPIFileNode({
                TrashTime: 123456,
            }), generateFileNode({
        ...
        it('should get nodes in batches', async () => {
            // @ts-expect-error Mocking for testing purposes
            apiMock.post = jest.fn(async (_, { LinkIDs }) => Promise.resolve({
                Links: LinkIDs.map((linkId) => generateAPIFolderNode({ LinkID: linkId })),
            }));
            ...
            expect(apiMock.post).toHaveBeenCalledWith('drive/v2/volumes/volumeId1/links', { LinkIDs: nodeIds.slice(0, 100) }, undefined);
            expect(apiMock.post).toHaveBeenCalledWith('drive/v2/volumes/volumeId1/links', { LinkIDs: nodeIds.slice(100, 200) }, undefined);
            expect(apiMock.post).toHaveBeenCalledWith('drive/v2/volumes/volumeId1/links', { LinkIDs: nodeIds.slice(200, 250) }, undefined);
        });
    });
    describe('trashNodes', () => {
        ...
        it('should trash nodes in batches', async () => {
            ...
            expect(apiMock.post).toHaveBeenCalledTimes(3);
            expect(apiMock.post).toHaveBeenCalledWith('drive/v2/volumes/volumeId1/trash_multiple', { LinkIDs: nodeIds.slice(0, 100) }, undefined);
            expect(apiMock.post).toHaveBeenCalledWith('drive/v2/volumes/volumeId1/trash_multiple', { LinkIDs: nodeIds.slice(100, 200) }, undefined);
            expect(apiMock.post).toHaveBeenCalledWith('drive/v2/volumes/volumeId1/trash_multiple', { LinkIDs: nodeIds.slice(200, 250) }, undefined);
        });
    });
    describe('restoreNodes', () => {
        ...
        it('should restore nodes from multiple volumes', async () => {
            // @ts-expect-error Mocking for testing purposes
            apiMock.put = jest.fn(async (_, { LinkIDs }) => Promise.resolve({
                Responses: LinkIDs.map((linkId) => ({
                    LinkID: linkId,
                    Response: {
                        Code: 1000 /* ErrorCode.OK */,
    ...
    describe('createFolder', () => {
        ...
        it('should throw NodeWithSameNameExistsValidationError if node already exists', async () => {
            apiMock.post = jest.fn().mockRejectedValue(new errors_1.ValidationError('Node already exists', 2500 /* ErrorCode.ALREADY_EXISTS */, {
            ...
            try {
                ...
                expect(error).toBeInstanceOf(errors_1.NodeWithSameNameExistsValidationError);
                if (error instanceof errors_1.NodeWithSameNameExistsValidationError) {
                    expect(error.code).toEqual(2500 /* ErrorCode.ALREADY_EXISTS */);
    ...
    describe('renameNode', () => {
        it('should rename node', async () => {
            await api.renameNode('volumeId~nodeId1', { hash: 'originalHash' }, { encryptedName: 'encryptedName1', nameSignatureEmail: 'nameSignatureEmail1', hash: 'newHash' });
            expect(apiMock.put).toHaveBeenCalledWith('drive/v2/volumes/volumeId/links/nodeId1/rename', {
                Name: 'encryptedName1',
        ...
        it('should throw error if node is out of sync', async () => {
            apiMock.put = jest.fn().mockRejectedValue(new apiService_1.InvalidRequirementsAPIError('Node is out of sync'));
            await expect(api.renameNode('volumeId~nodeId1', { hash: 'originalHash' }, { encryptedName: 'encryptedName1', nameSignatureEmail: 'nameSignatureEmail1', hash: 'newHash' })).rejects.toThrow(new errors_2.NodeOutOfSyncError('Node is out of sync'));
        });
    });
});
describe('groupNodeUidsByVolumeAndIteratePerBatch', () => {
    it('should handle empty array', () => {
        const result = Array.from((0, apiService_2.groupNodeUidsByVolumeAndIteratePerBatch)([]));
        expect(result).toEqual([]);
    });
    it('should handle single volume with nodes that fit in one batch', () => {
        ...
        const result = Array.from((0, apiService_2.groupNodeUidsByVolumeAndIteratePerBatch)(nodeUids));
        expect(result).toEqual([
            {
                volumeId: 'volumeId1',
