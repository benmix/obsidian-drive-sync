...
describe('extended attrbiutes', () => {
    describe('should generate folder attributes', () => {
        ...
        testCases.forEach(([input, expectedAttributes]) => {
            it(`should generate ${input}`, () => {
                const output = (0, extendedAttributes_1.generateFolderExtendedAttributes)(input);
    ...
    describe('should parse folder attributes', () => {
        ...
        testCases.forEach(([input, expectedAttributes]) => {
            it(`should parse ${input}`, () => {
                const output = (0, extendedAttributes_1.parseFolderExtendedAttributes)((0, logger_1.getMockLogger)(), input);
    ...
    describe('should generate file attributes without additional metadata', () => {
        ...
        testCases.forEach(([input, expectedAttributes]) => {
            it(`should generate ${input}`, () => {
                const output = (0, extendedAttributes_1.generateFileExtendedAttributes)(input);
                expect(output).toBe(expectedAttributes);
            });
        });
    });
    describe('should generate file attributes with additional metadata', () => {
        ...
        testCases.forEach(([input, expectedAttributes]) => {
            it(`should generate ${input}`, () => {
                const output = (0, extendedAttributes_1.generateFileExtendedAttributes)(input, { Media: { Width: 100, Height: 100 } });
                expect(output).toBe(expectedAttributes);
            });
        });
    });
    describe('should throw an error if additional metadata contains common attributes', () => {
        it('should throw an error', () => {
            expect(() => (0, extendedAttributes_1.generateFileExtendedAttributes)({ size: 123 }, { Common: { Hello: 'World' } })).toThrow('Common attributes are not allowed in additional metadata');
        });
    });
    describe('should parses file attributes', () => {
        ...
        testCases.forEach(([creationTime, input, expectedAttributes]) => {
            it(`should parse ${input}`, () => {
                const output = (0, extendedAttributes_1.parseFileExtendedAttributes)((0, logger_1.getMockLogger)(), creationTime, input);
                expect(output).toMatchObject(expectedAttributes);
            });
        });
    });
});
//# sourceMappingURL=extendedAttributes.test.js.map