...
describe('nodesCryptoService', () => {
    ...
    const ownPrivateAddressKey = { id: 'id', key: 'key' };
    beforeEach(() => {
        jest.clearAllMocks();
        ...
        driveCrypto = {
            ...
            verifyInvitation: jest.fn(async () => Promise.resolve({
                verified: crypto_1.VERIFICATION_STATUS.SIGNED_AND_VALID,
            })),
        };
        ...
        const nodesCryptoReporter = new cryptoReporter_1.NodesCryptoReporter(telemetry, sharesService);
        cryptoService = new cryptoService_1.NodesCryptoService(telemetry, driveCrypto, account, nodesCryptoReporter);
    });
            ...
            encryptedNode = {
                ...
                encryptedCrypto: {
                    ...
                    membership: {
                        ...
                        armoredInviterSharePassphraseKeyPacketSignature: 'armoredInviterSharePassphraseKeyPacketSignature',
                        armoredInviteeSharePassphraseSessionKeySignature: 'armoredInviteeSharePassphraseSessionKeySignature',
                    },
                },
            };
        });
    ...
    describe('file node', () => {
        ...
        describe('should decrypt with verification issues', () => {
            ...
            it('on content key packet with successful fallback verification', async () => {
                ...
                expect(driveCrypto.decryptAndVerifySessionKey).toHaveBeenCalledWith('base64ContentKeyPacket', 'armoredContentKeyPacketSignature', 'decryptedKey', ['decryptedKey', publicAddressKey]);
                expect(driveCrypto.decryptAndVerifySessionKey).toHaveBeenCalledWith('base64ContentKeyPacket', 'armoredContentKeyPacketSignature', 'decryptedKey', [ownPrivateAddressKey.key]);
                expect(telemetry.recordMetric).not.toHaveBeenCalled();
            });
    ...
    describe('anonymous node', () => {
        ...
        describe('should decrypt with verification issues', () => {
            ...
            it('on anonymous node key and name without access to parent node', async () => {
                ...
                expect(telemetry.recordMetric).not.toHaveBeenCalled();
                expect(driveCrypto.decryptKey).toHaveBeenCalledWith(encryptedNode.encryptedCrypto.armoredKey, encryptedNode.encryptedCrypto.armoredNodePassphrase, encryptedNode.encryptedCrypto.armoredNodePassphraseSignature, [parentKey], []);
                expect(driveCrypto.decryptNodeName).toHaveBeenCalledWith(encryptedNode.encryptedName, parentKey, []);
            });
        });
    });
    describe('createFolder', () => {
        ...
        it('should encrypt new folder with account key', async () => {
            ...
            const result = await cryptoService.createFolder(parentKeys, signingKeys, 'New Folder', '{"modificationTime": 1234567890}');
            ...
            expect(driveCrypto.encryptNodeName).toHaveBeenCalledWith('New Folder', undefined, parentKeys.key, signingKeys.key);
            ...
            expect(driveCrypto.encryptExtendedAttributes).toHaveBeenCalledWith('{"modificationTime": 1234567890}', 'nodeKey', signingKeys.key);
        });
        it('should encrypt new folder with node key', async () => {
            ...
            expect(driveCrypto.encryptNodeName).toHaveBeenCalledWith('New Folder', undefined, parentKeys.key, signingKeys.parentNodeKey);
            expect(driveCrypto.generateLookupHash).toHaveBeenCalledWith('New Folder', parentKeys.hashKey);
            expect(driveCrypto.generateHashKey).toHaveBeenCalledWith('nodeKey');
            expect(driveCrypto.encryptExtendedAttributes).toHaveBeenCalledWith('{"modificationTime": 1234567890}', 'nodeKey', signingKeys.nodeKey);
        });
    });
    describe('encryptNewName', () => {
        ...
        it('should encrypt new name with account key', async () => {
            ...
            const result = await cryptoService.encryptNewName(parentKeys, nodeNameSessionKey, signingKeys, 'Renamed File.txt');
            expect(result).toEqual({
                signatureEmail: 'test@example.com',
                armoredNodeName: 'encryptedNewNodeName',
                hash: 'newHash',
            });
            expect(driveCrypto.encryptNodeName).toHaveBeenCalledWith('Renamed File.txt', nodeNameSessionKey, parentKeys.key, signingKeys.key);
        ...
        it('should encrypt new name with node key', async () => {
            ...
            expect(driveCrypto.encryptNodeName).toHaveBeenCalledWith('Renamed File.txt', nodeNameSessionKey, parentKeys.key, signingKeys.parentNodeKey);
            expect(driveCrypto.generateLookupHash).toHaveBeenCalledWith('Renamed File.txt', parentKeys.hashKey);
        });
    });
    describe('encryptNodeWithNewParent', () => {
        ...
        it('should encrypt node data for move operation with account key (logged in context)', async () => {
            ...
            const result = await cryptoService.encryptNodeWithNewParent(node.name, keys, parentKeys, signingKeys);
            expect(result).toEqual({
                encryptedName: 'encryptedNodeName',
                hash: 'newHash',
                armoredNodePassphrase: 'encryptedPassphrase',
            ...
            expect(driveCrypto.encryptNodeName).toHaveBeenCalledWith('testFile.txt', keys.nameSessionKey, parentKeys.key, signingKeys.key);
            expect(driveCrypto.generateLookupHash).toHaveBeenCalledWith('testFile.txt', parentKeys.hashKey);
            expect(driveCrypto.encryptPassphrase).toHaveBeenCalledWith(keys.passphrase, keys.passphraseSessionKey, [parentKeys.key], signingKeys.key);
        });
        it('should encrypt node data for move operation with node key (anonymous context)', async () => {
            ...
            expect(driveCrypto.encryptNodeName).toHaveBeenCalledWith('testFile.txt', keys.nameSessionKey, parentKeys.key, signingKeys.nodeKey);
            expect(driveCrypto.generateLookupHash).toHaveBeenCalledWith('testFile.txt', parentKeys.hashKey);
            expect(driveCrypto.encryptPassphrase).toHaveBeenCalledWith(keys.passphrase, keys.passphraseSessionKey, [parentKeys.key], signingKeys.nodeKey);
        });
        it('should throw error when moving to non-folder', async () => {
            ...
            await expect(cryptoService.encryptNodeWithNewParent(node.name, keys, parentKeys, signingKeys)).rejects.toThrow('Moving item to a non-folder is not allowed');
        });
        it('should throw error when node has invalid name', async () => {
            ...
            await expect(cryptoService.encryptNodeWithNewParent(node, keys, parentKeys, signingKeys)).rejects.toThrow('Cannot move item without a valid name, please rename the item first');
        });
    });
});
